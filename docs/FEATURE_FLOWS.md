...
12. **`/webhook/{tier}` receiver** — `wildwatch/webhooks.py:receive_alert` accepts the POST, validates the payload schema (`AlertPayload`), and starts fan-out. **Optional auth:** when `WILDWATCH_WEBHOOK_SECRET` is set, requests without a matching `X-WildWatch-Secret` header are rejected with 401 (verified via `hmac.compare_digest`). Path-B sweep + correlation runner forward the header. Unset → loud startup WARNING + open access (back-compat for localhost-only demo).
...
6. **POST endpoint** — `webhooks.py:api_create_source` (JSON) or `webhooks.py:api_upload_source` (multipart). The JSON path runs through Pydantic's `SourceCreate` model whose `@field_validator` blocks SSRF: per-kind URL-scheme allowlist (rtsp→rtsp://, youtube→youtube.com/youtu.be, etc.), hard reject on `file:` / `gopher:` / `javascript:`, and the `_host_is_private(host)` helper that layers stdlib `ipaddress` parsing over a regex so loopback / private / link-local IPs are blocked in both textual (`127.0.0.1`) AND IPv4-mapped IPv6 (`::ffff:127.0.0.1`) notation. Length caps (input 2000, name 200) prevent flooded payloads.
...
14d. **Live YouTube** — `_ingest_youtube` probes `yt-dlp --print is_live` before calling `coll.upload(url=...)`. If the URL is a live stream, the dispatch raises a clear `RuntimeError` pointing at the docker-compose bridge. The operator runs `docker compose -f bridge/docker-compose.yml up -d` (mediamtx + bore), then `./bridge/start_bridge.sh "<url>" <slug>`, reads the public `rtsp://bore.pub:<port>/<slug>` URL from the bore container's logs, and adds that as a new RTSP source via the dashboard's +Add modal.
