...
14d. **Live YouTube → bridge helper (auto-detect)** — `_ingest_youtube` probes `yt-dlp --print is_live` before calling `coll.upload(url=...)`. If the URL is a live stream, VideoDB can't read it directly (only `rtsp://` / `rtmp://` for live). Instead of erroring, the source is parked in a new `needs_bridge` status with two new fields populated: `bridge_command` (the shell line the operator needs to run — `./bridge/start_bridge.sh "<url>" <slug>`) and `bridge_rtsp` (`rtsp://localhost:8554/<slug>`). The dashboard's `renderSource` renders a yellow-bordered helper card with a copy-button on the command + an input pre-filled with the suggested RTSP URL + a "Use bridge" button. The operator runs the command in a second terminal (mediamtx + streamlink + ffmpeg), then submits the URL via `POST /api/sources/{id}/use-bridge`. That endpoint flips `kind` from `youtube` to `rtsp`, replaces `input` with the pasted URL, and re-dispatches ingest — from there the source behaves identically to any other RTSP source.
