...
6. **POST endpoint** — `webhooks.py:api_create_source` (JSON) or `webhooks.py:api_upload_source` (multipart). The JSON path runs through Pydantic's `SourceCreate` model whose `@field_validator` blocks SSRF: per-kind URL-scheme allowlist (rtsp→rtsp://, youtube→youtube.com/youtu.be, etc.), hard reject on `file:` / `gopher:` / `javascript:`, regex-block on RFC-1918 / link-local / loopback hosts. Length caps (input 2000, name 200) prevent flooded payloads.
