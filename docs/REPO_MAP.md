...
| What it is | `send_alert(...)` posts an HTML-formatted message to your Telegram chat with the alert label, a one-line GenAI-rewritten explanation, and a short tappable "Play clip" link. |
| --- | --- |
| Why it exists | A live demo where the presenter's phone buzzes mid-presentation is what sells "real-time perception." Ranger-friendly text + a hidden long URL is what sells it to non-tech audiences. |
