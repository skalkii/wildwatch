...
| **Sources** | Add any source: file upload (≤500 MB), URL (YouTube / HLS), or RTSP/RTMP. Each card shows live status (`queued` → `connecting` → `ingesting` → `indexing` → `ready`). Uploads auto-trigger BOTH a visual scene index (species) AND an audio index, then a background sweep searches each index for gunshot / chainsaw / rare-species / alarm-call patterns and fires Telegram alerts on hits — no cloudflared tunnel needed. Per-kind actions: live streams get **Reconnect / Disconnect / Delete**; uploaded files and URL sources get **Re-index / Delete** (re-uploading would just duplicate the video). |
